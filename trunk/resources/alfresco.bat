...
rem set DEBUG_OPTS= -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=8000
rem JAVA_OPTS
rem =-Xms512m -Xmx1024m -Xss1024k -XX:MaxPermSize=256m -XX:NewSize=256m -server -Dalfresco.home=%ALF_HOME% -Dcom.sun.management.jmxremote  -Duser.language=en -Duser.country=en  -Duser.region=US 
...
:getpaths
rem call RegPaths.exe
rem call SetPaths.bat
rem del SetPaths.bat
...
rem start "MySQL" "%ALF_HOME%mysql\bin\mysqld" --defaults-file="%ALF_HOME%mysql\my.ini" --basedir="%ALF_HOME%mysql" --datadir="%ALF_HOME%alf_data\mysql" --console
...
rem call "%ALF_HOME%mysql\bin\mysqladmin" -u root shutdown
